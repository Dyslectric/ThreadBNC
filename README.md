...
- **The YouTube page** (`/youtube`, linked from Accounts) sets the largest video downloaded (2000 MB by default) and the best quality (1080p). After that, the **Videos** media settings apply as to any video: one bigger than it's kept as it is up to is transcoded down to a size or re-encoded at a bitrate, if one is chosen, and kept as it downloaded if not (never left out for its size). Changing those settings converts YouTube videos already saved too. A community that doesn't archive videos saves no YouTube videos.
